...
import net.sourceforge.peers.sip.core.useragent.ConfigManager;
    ...
    implements ClientTransactionUser
{
        ...
        throws SipUriSyntaxException
    {
        ...
        sipHeaders.add(new SipHeaderFieldName(RFC3261.HDR_EXPIRES),
            new SipHeaderFieldValue(isUnregisterInvoked() ?
                ConfigManager.getInstance().getSipRequestsHeaderConfig().getUnregisterExpiresTime().toString() :
                String.valueOf(
        ...
        Transaction transaction)
    {
        ...
        if (!"0".equals(expires))
        {
            ...
            if (userAgent.getConfig().useCustomRegisterPeriod())
                delay = userAgent.getConfig().getCustomRegisterPeriod();
            else
                delay = Utils.isEmptyString(expires) ? 3600 : Integer.parseInt(expires) - REFRESH_MARGIN;
    ...
    public boolean isUnregisterInvoked()
    {
        return unregisterInvoked;
    }
